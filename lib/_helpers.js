...
export function calculateProbabilities(navigation) {
  ...
  // Inicializar la matriz invertida con valores de probabilidad cero
  for (const fromRoute in probabilityMatrix) {
    invertedMatrix[fromRoute] = {}
    for (const toRoute in probabilityMatrix[fromRoute]) {
      invertedMatrix[fromRoute][toRoute] = 0
    }
  }

  // Calcular las probabilidades invertidas
  for (const fromRoute in probabilityMatrix) {
    const totalIncomingTransitions = Object.values(probabilityMatrix).reduce((acc, val) => acc + val[fromRoute], 0)
    for (const toRoute in probabilityMatrix[fromRoute]) {
      if (totalIncomingTransitions === 0) {
        invertedMatrix[fromRoute][toRoute] = 0
      } else {
        invertedMatrix[fromRoute][toRoute] = probabilityMatrix[toRoute][fromRoute] / totalIncomingTransitions
      }
    }
  }
  ...
  for (const route1 of routes) {
    const rowProbabilities = Object.values(invertedMatrix[route1])
  ...
  // console.log(JSON.stringify(invertedMatrix, null, 2))
